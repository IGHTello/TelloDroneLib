...
struct DronePacket {
    ...
    DronePacket(u16 seq_num, u8 packet_type, CommandID cmd_id, std::vector<u8> data)
        : seq_num(seq_num), packet_type(packet_type), cmd_id(cmd_id), data(std::move(data)), direction(PacketDirection::FROM_DRONE) {}

	DronePacket(u8 packet_type, CommandID cmd_id, std::vector<u8> data = {})
		: seq_num(-1), packet_type(packet_type), cmd_id(cmd_id), data(std::move(data)), direction(PacketDirection::TO_DRONE) {}

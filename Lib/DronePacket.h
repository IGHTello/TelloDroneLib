...
struct DronePacket {
    ...
    DronePacket(u16 seq_num, u8 packet_type, CommandID cmd_id, std::vector<u8> data, bool to_drone)
        : seq_num(seq_num), packet_type(packet_type), cmd_id(cmd_id), data(std::move(data)), to_drone(to_drone) {}

    std::vector<u8> serialize();
    static std::optional<DronePacket> deserialize(std::span<u8> packet_bytes, bool to_drone);
};
